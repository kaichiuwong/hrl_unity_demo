...
public class AgentSoccer : Agent
{
    ...
    public override void CollectObservations()
    {
        ...
        /*
        string[] teammateobj;
        string[] ballobj = new string[] { "ball" };
        if (team == Team.red)
        {
            teammateobj = new string[] { "redTeammate" };
        }
        else
        {
            teammateobj = new string[] { "blueTeammate" };
        }
        List<float> teamPer = rayPer.Perceive(rayDistance, rayAngles, teammateobj, 0f, 0f);
        List<float> ballPer = rayPer.Perceive(rayDistance, rayAngles, ballobj, 0f, 0f);
        float teamdistance = 0;
        float teamangle = 0;
        for (int i = 0; i < rayAngles.Length; i++ )
        {
            ...
            if (teamPer[idx] > 0)
            {
                teamdistance = teamPer[idx];
                teamangle = rayAngles[i];
                break;
            }
        }
        ...
        for (int i = 0; i < rayAngles.Length; i++)
        {
            ...
            if (ballPer[idx] > 0)
            {
                balldistance = ballPer[idx];
                ballangle = rayAngles[i];
                break;
            }
        }

        float x= balldistance * Mathf.Sin(teamangle);
        float y= balldistance * Mathf.Cos(teamangle);
        float[] ord = { x, y };
        AddVectorObs(ord);
        */
    }
