...
public class TennisAgent : Agent
{
    ...
    public override void CollectObservations()
    {
        ...
        AddVectorObs(teammate.invertMult * (teammate.transform.position.x - teammate.myArea.transform.position.x));
        AddVectorObs(teammate.transform.position.y - teammate.myArea.transform.position.y);
        AddVectorObs(teammate.invertMult * teammate.agentRb.velocity.x);
        AddVectorObs(teammate.agentRb.velocity.y);
    }
    ...
    public override void AgentAction(float[] vectorAction, string textAction)
    {
        ...
        agentRb.velocity = new Vector3(moveX * 30f, 0f, 0f);
    ...
    public override void AgentReset()
    {
        ...
        if (name.Contains("1"))
        {
            transform.position = new Vector3(-invertMult * Random.Range(0f, 5f), -1.5f, 0f) + transform.parent.transform.position;
        }
        else
        {
            transform.position = new Vector3(-invertMult * Random.Range(5f, 10f), -1.5f, 0f) + transform.parent.transform.position;
        }
