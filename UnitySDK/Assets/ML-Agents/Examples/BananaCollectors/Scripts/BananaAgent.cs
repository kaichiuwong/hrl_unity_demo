...
public class BananaAgent : Agent
{
    ...
    public void MoveAgent(float[] act)
    {
        ...
        if (Time.time > frozenTime + 20f && frozen)
        {
            Unfreeze();
        }
        if (Time.time > effectTime + 20f)
        {
            if (poisioned)
            {
                Unpoison();
    ...
    void OnCollisionEnter(Collision collision)
    {
        if (collision.gameObject.CompareTag("banana"))
        {
            ...
            if (contribute)
            {
                ...
                {
                    myAcademy.redtotalScore += 1;
                }
                else
                {
                    myAcademy.bluetotalScore += 1;
                }
            }
        }
        ...
        Debug.Log(string.Format("{0:HH:mm:ss}, Blue: {1}, Red: {2}", System.DateTime.Now, myAcademy.bluetotalScore, myAcademy.redtotalScore));
        if (this.name.Contains("Red"))
        {
            txt.text = string.Format(@"{0}", myAcademy.redtotalScore);
        }
        else
        {
            txt.text = string.Format(@"{0}", myAcademy.bluetotalScore);
        }
            //myAcademy.SetText(string.Format(@"Score: {0}", myAcademy.totalScore));
    }
