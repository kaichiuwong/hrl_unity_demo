...
public class CrawlerAgent : Agent
{
    ...
    public void GetRandomTargetPos()
    {
        ...
        newTargetPos.y = 5;
        target.transform.position = newTargetPos + ground.position;
    }
    ...
    public override void AgentAction(float[] vectorAction, string textAction)
    {
        ...
        float distanceBetweenTarget = Vector3.Distance(body.position, target.body.position);
        if (distanceBetweenTarget > 7)
        {
            target.AgentReset(false);
            AgentReset(false);
        }

        // Update pos to target
        dirToTarget = target.transform.position - jdController.bodyPartsDict[body].rb.position;
    ...
    public void AgentReset(bool update)
    {
        ...
        if (update)
        {
            target.scoreText.GetComponent<Text>().text = string.Format("{0}", ++target.score);
            if (this.name.Contains("Blue"))
            {
                Debug.Log(string.Format("{0:HH:mm:ss}, Blue: {1}, Red: {2}", System.DateTime.Now, score, target.score));
            }
            else
            {
                Debug.Log(string.Format("{0:HH:mm:ss}, Blue: {1}, Red: {2}", System.DateTime.Now, target.score, score));
            }
        }
    }
}
